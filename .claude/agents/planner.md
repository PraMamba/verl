...
- **Designing new features** (worker, dataset, reward, algorithm)
...
1. **Find similar implementations**
   ...
   - Search for classes/functions with similar patterns:
     `grep "class.*Worker" verl/workers/`
...
1. **Check tests**
   ...
   - Does the target file have tests? `ls tests/unit/test_<module>.py`
   - What test patterns are used? Read a test file for reference

1. **Check configuration**

   - Does this involve Hydra configs in `verl/trainer/config/`?
   - Are there config dataclasses in `verl/workers/config/` to modify?
