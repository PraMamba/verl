...
Codex can run subagent workflows by spawning specialized agents in parallel and then collecting their results in one response. This is particularly useful for complex tasks that are highly parallel, such as codebase exploration or implementing a multi-step feature plan.
...
- Subagent activity is currently surfaced in the Codex app and CLI.
- Visibility in the IDE Extension is coming soon.
...
Codex handles orchestration across agents, including:
...
When omitted, the following inherit from the parent session:
...
- `agents.max_threads` defaults to `6` when unset.
- `agents.max_depth` defaults to `1`, which allows a direct child agent to spawn but prevents deeper nesting.
- `agents.job_max_runtime_seconds` is optional. When unset, `spawn_agents_on_csv` falls back to its per-call default timeout of `1800` seconds per worker.
...
The best custom agents are narrow and opinionated. Give each one:

- a clear job
- a tool surface that matches that job
- instructions that keep it from drifting into adjacent work
...
When you run this through `codex exec`, Codex shows a single-line progress update on `stderr` while the batch is running.

The exported CSV includes the original row data plus metadata such as:
...
- `agents.job_max_runtime_seconds` sets the default per-worker timeout for CSV fan-out jobs.
- A per-call `max_runtime_seconds` override takes precedence.
...
- UI regressions
- flaky browser flows
- integration bugs that cross application code and the running product
...
Subagent workflows in Codex are designed for explicit, parallel task execution. They are most effective when:

- the work can be decomposed into narrow, independent tasks
- each agent has a clear responsibility
- custom agents are configured with focused instructions and appropriate tool access

Use built-in agents for common cases, and define project-specific custom agents when you need stronger specialization, stricter sandboxing, or different model tradeoffs.
