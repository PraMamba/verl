...
| 注册键（6 个） | `naive`, `nccl`, `nixl`, `kimi_ckpt_engine`, `mooncake`, `delta_sharded` |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `__init__.py` | 73 | 条件导入与公开 API (`__all__`)，所有后端均用 `try/except ImportError` 防护 |
| `base.py` | 609 | 核心抽象层：`TensorMeta`、`CheckpointEngineRegistry`、`CheckpointEngine` ABC、`CheckpointEngineWithCache`、`ColocatedCheckpointEngine`、`CheckpointEngineWorker`、`CheckpointEngineManager`、以及 `split_weight_chunks` / `merge_weight_chunks` 辅助函数 |
| `nccl_checkpoint_engine.py` | 387 | `NCCLCheckpointEngine` -- 基于 NCCL 集合通信的广播传输 |
| `hccl_checkpoint_engine.py` | 396 | `HCCLCheckpointEngine` -- 华为 Ascend NPU 的 HCCL 集合通信 |
| `kimi_checkpoint_engine.py` | 391 | `KIMICheckpointEngine` -- 月之暗面定制的 ParameterServer P2P 传输 |
| `mooncake_checkpoint_engine.py` | 321 | `MooncakeCheckpointEngine` -- 基于 Mooncake TransferEngine 的 RDMA P2P 传输 |
| `nixl_checkpoint_engine.py` | 532 | `NIXLCheckpointEngine` -- NVIDIA NIXL 链式 P2P 传输 |
| `delta_checkpoint_engine.py` | 666 | `DeltaShardedCheckpointEngine` -- 继承 NCCL, delta (byte-diff) 稀疏权重同步 |
| `delta_sync/__init__.py` | 32 | delta_sync 子包导出 (`DeltaFlush` / `DeltaParam` / `checksum`) |
| `delta_sync/encode.py` | 91 | delta 编码: `DeltaParam` / `DeltaFlush` 数据结构与 payload checksum |
| `delta_sync/sparse_gather.py` | 156 | 稀疏 (position, value) 对的收集与散射 |
...
已注册的后端名称（注册键）：
- `"naive"` -- `ColocatedCheckpointEngine`（base.py:226，注册于 :225）
- `"nccl"` -- `NCCLCheckpointEngine`（nccl_checkpoint_engine.py:104，注册装饰器 @103）或 `HCCLCheckpointEngine`（hccl_checkpoint_engine.py:107，注册装饰器 @106，覆盖同名注册）
...
- `"mooncake"` -- `MooncakeCheckpointEngine`（mooncake_checkpoint_engine.py:38）
- `"delta_sharded"` -- `DeltaShardedCheckpointEngine`（delta_checkpoint_engine.py:219，注册于 :218，继承 NCCL）

注意：这里的 6 个是注册键而非 6 个互不相同的键名实现；`HCCLCheckpointEngine` 也注册为 `"nccl"`，在华为 Ascend 环境下会覆盖 NCCL 后端。因此实现类可同时列出 NCCL/HCCL，但二者共享一个 registry key。
...
### 4.2 CheckpointEngineWithCache (base.py:208-223)
...
### 4.3 ColocatedCheckpointEngine (base.py:226-281)
...
CheckpointEngine (ABC)
  ...
  |     |-- DeltaShardedCheckpointEngine [delta_sharded] (delta 稀疏同步, 复用 NCCL 组)
...
### 5.1 CheckpointEngineWorker (base.py:283-359)

`CheckpointEngineWorker` 继承自 `Worker`（verl 的 Ray Worker 基类），与推理引擎的 `WorkerProc` 共处同一 GPU。关键逻辑：

- **构造函数**（base.py:292-332）：从 `rollout_config.checkpoint_engine` 读取配置，通过 `update_weights_bucket_megabytes << 20` 将 MB 转换为字节（base.py:313），调用 `CheckpointEngineRegistry.new()` 创建后端实例。同时支持自定义后端模块的动态导入（`import_external_libs`）。
- **`update_weights`**（base.py:335）：`@register(dispatch_mode=Dispatch.ONE_TO_ALL, blocking=False)` 注册的异步方法，从 checkpoint engine 接收权重后调用 `server_adapter.update_weights()` 更新推理引擎。
- **`execute_checkpoint_engine`**（base.py:344）：`@register(dispatch_mode=Dispatch.DP_COMPUTE, blocking=False)` 注册的通用方法分派器，通过 `getattr(self.checkpoint_engine, method)` 转发任意方法调用。

### 5.2 CheckpointEngineManager (base.py:361-538)
...
核心流程 `update_weights`（base.py:486-538）分 8 步：
...
`build_process_group`（base.py:403）的三阶段：
1. 所有 worker 调用 `prepare`，收集元数据
2. 调用 `backend_cls.build_topology()` 计算拓扑
3. 所有 worker 调用 `init_process_group` 建立连接

弹性伸缩支持：`add_replicas`（base.py:430）和 `remove_replicas`（base.py:438）允许动态增减 rollout replica。

### 5.3 Bucket 分块辅助函数

**`split_weight_chunks`**（base.py:541）：将权重 generator 按 `bucket_size` 分块。每个权重先 `view(-1).view(torch.uint8)` 展平为字节流，然后按 bucket 容量切割，生成 `(TensorMeta, chunk)` 元组。

**`merge_weight_chunks`**（base.py:570）：反向操作。若单个权重小于 bucket，直接 `view(dtype).view(shape)` 还原；若跨多个 chunk，则分配临时 buffer 逐块拼接。通过 `chunk_offset` 和 `chunk_size` 的断言保证顺序正确。

## 6. 传输后端详解

### 6.1 NCCLCheckpointEngine (nccl_checkpoint_engine.py:104-387; 注册装饰器 @103)
...
### 6.2 HCCLCheckpointEngine (hccl_checkpoint_engine.py:107-396)

华为 Ascend NPU 的对应实现，与 NCCL 版本结构几乎一致，关键差异：
- 使用 `torch.npu` 替代 `torch.cuda`（hccl_checkpoint_engine.py:132）
- 使用 `StatelessProcessGroup` 进行进程组初始化而非 `ray.util.collective`（hccl_checkpoint_engine.py:219-221）
- `BroadcastOperation._run` 直接执行而非在线程池中运行（hccl_checkpoint_engine.py:83）
- `MasterMetadata` 增加 `dist_ip` 和 `dist_port` 字段（hccl_checkpoint_engine.py:45-49）
...
### 6.4 MooncakeCheckpointEngine (mooncake_checkpoint_engine.py:39-321)

基于 Mooncake TransferEngine 的 RDMA 传输，特点：
- **P2P 链式传输**：rank 0 -> rank 1 -> ... -> rank N-1 的链式传输拓扑
- **TransferEngine 初始化**（mooncake_checkpoint_engine.py:73-74）：使用 `"P2PHANDSHAKE"` 模式，根据设备类型选择 `"ascend_direct"` 或 `"rdma"` 传输后端
- **双 bucket 分区**：`self.buf = torch.empty(2 * self.bucket_size, ...)` 分为两段交替使用（mooncake_checkpoint_engine.py:83）
- **Magic 同步**：使用 4 字节 magic 值 `[0xAB, 0xDC, 0xEF, 0x88]` 作为完成信号（mooncake_checkpoint_engine.py:150, 252）。发送端通过轮询 buffer 头部的 magic 值来判断接收端是否已读取完毕
...
### 6.6 DeltaShardedCheckpointEngine (delta_checkpoint_engine.py:219-666)

`class DeltaShardedCheckpointEngine(NCCLCheckpointEngine)`（delta_checkpoint_engine.py:219，注册名 `"delta_sharded"` @218）继承 NCCL 广播后端，但把 **delta（byte-diff）而非全量权重** 放到 trainer->rollout 链路上：trainer 对后端维护的快照做字节差分，只广播变化的 `(position, value)` 稀疏对，复用与全量同步相同的 `ray.util.collective` NCCL 组（actor rank0 -> rollout `CheckpointEngineWorker`）。快照驻留位置由后端决定；例如 Megatron 的 `delta_pin_snapshots=False`（`transformer_impl.py:78-84`），因此不能概括为统一的 pinned-CPU 快照。该后端当前只允许 SGLang rollout，其他 rollout 会在 `CheckpointEngineWorker` 初始化时 fail-fast（`checkpoint_engine/base.py:305-312`）。

**`delta_sync/` 子包**（共 279 行，是本模块相对基线 +40% 增长的主体）：
- `delta_sync/__init__.py`（32 行）：导出 `DeltaFlush` / `DeltaParam` / `checksum`
- `delta_sync/encode.py`（91 行）：`DeltaParam` / `DeltaFlush` 数据结构与 payload checksum 计算。`DeltaParam`（L34-50）记录参数名、dtype/shape、positions 与 values 的区间；`DeltaFlush`（L66-91）携带 encoding、参数 manifest、positions/values 张量和 checksum，并提供 `nnz`/`wire_bytes` 派生属性
- `delta_sync/sparse_gather.py`（156 行）：稀疏 `(position, value)` 对的收集与散射

rollout 侧的对应件是 `sglang_rollout/delta_loader.py` 的 `apply_delta()`（属 06 模块）：rollout worker 收到稀疏 payload 后就地打补丁到本地权重副本。delta 相关 4 文件（`delta_checkpoint_engine.py` 666 + `delta_sync/` 279）共 **945 行**。
...
Manager 的 `update_weights` 流程中内置了 partial rollout 支持（`abort_replicas()` @ base.py:499、`resume_generation_replicas()` @ base.py:536）：
