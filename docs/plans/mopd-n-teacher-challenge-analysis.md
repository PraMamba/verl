...
同时也要区分“schema 已声明”和“能力边界必须写清”：

- `TeacherConfig.weight` 仍不是当前 runtime 能力；非默认值现在会 fail-fast，而不是静默忽略
- `TeacherConfig.base_model_path` 也仍不是当前 runtime 能力；ExOPD 只消费全局
  `algorithm.mopd.base_model_path` 这一条 shared base 配置，而不是 per-teacher base

这里还有一个必须单独点出来的现实细节：

- `TeacherConfig` / `MOPDConfig` 确实存在，并实现了 `__post_init__` 校验
- `AlgoConfig.__post_init__` 现在也会把 `mopd` 材料化成 `MOPDConfig`
- `validate_config()` 已会先实例化 `AlgoConfig`，因此 runtime typed validation 已经接通
- 当前残留的不足是 `AlgoConfig.mopd` 注解仍为 `Any`，所以 Hydra composition-time strictness 仍弱于 actor/critic

这意味着：

- “配置语义重载”已经解决。
- “runtime typed config 闭环”已经解决。
- 仍未完全收尾的是 “Hydra composition-time typed strictness”。
...
- 更保守的长程 rerun：退出码 `0`，到达 `Training Progress: 100%|...| 18/18`、
...
- 一部分工程收尾辅助逻辑
- recipe-aligned preflight、GPU E2E、full recipe run 和保守 `18/18` long-run rerun 在最新 worktree 上都已 fresh 跑通
...
当前 `mopd` 的 runtime typed dataclass 校验已经接到主链路：

- `AlgoConfig.__post_init__` 会把 `mopd` 材料化成 `MOPDConfig`
- `validate_config()` 也会统一先实例化 `AlgoConfig`

所以这项挑战的“语义重载问题”已解决，runtime typed validation 也已补齐。
当前还没和 actor/critic 完全等价的地方，是 `AlgoConfig.mopd` 的注解仍为 `Any`，
因此 Hydra composition-time strictness 仍偏弱。
