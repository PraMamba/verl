...
def test_preflight_script_uses_rvq_v1_reward_contract():
    ...
    assert (
        'PREFLIGHT_REWARD_EST_TOKENS_PER_REQUEST="${PREFLIGHT_REWARD_EST_TOKENS_PER_REQUEST:-18000}"'
        in preflight_script
    )
...
def test_full_run_script_defaults_to_gpfs_output_and_runtime_roots():
    ...
    assert (
        'CKPTS_ROOT=${CKPTS_ROOT:-"${TRAINING_OUTPUT_ROOT}/Qwen3-4B-Thinking-2507_Manual_Resize_Block-8_Size-32_Num-256_ExplicitTokens_Continued-Pretraining-ALL_Augmented-V1_SFT-ALL_DFT-MOPD"}'
        in train_script
    )
...
def test_full_run_script_externalizes_runtime_temp_and_cache_roots():
    ...
    assert (
        'TORCHINDUCTOR_CACHE_DIR=${TORCHINDUCTOR_CACHE_DIR:-"${RUNTIME_CACHE_ROOT}/i/${RUNTIME_CACHE_TAG}"}'
        in train_script
    )
    ...
    assert (
        'TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR:-"${RUNTIME_CACHE_ROOT}/x/${RUNTIME_CACHE_TAG}"}' in train_script
    )
    assert (
        'FLASHINFER_WORKSPACE_BASE=${FLASHINFER_WORKSPACE_BASE:-"${RUNTIME_CACHE_ROOT}/fi/${RUNTIME_CACHE_TAG}"}'
        in train_script
    )
...
def test_main_launcher_mini_val_wrapper_applies_validation_first_defaults_under_gpfs():
    ...
    assert (
        'RUNTIME_ROOT=${RUNTIME_ROOT:-"${TRAINING_OUTPUT_ROOT}/mopd_qwen3_4b_mini_val_runtime/${RUN_ID}"}'
        in mini_val_script
    )
...
def test_mini_val_wrapper_exec_resolves_runtime_cache_and_output_roots(tmp_path):
    ...
    assert capture["env"]["RUNTIME_ROOT"] == str(training_output_root / "mopd_qwen3_4b_mini_val_runtime" / run_id)
